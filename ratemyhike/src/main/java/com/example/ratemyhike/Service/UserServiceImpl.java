...
public class UserServiceImpl implements UserService{
    ...
    @Override
    public User getUserById(long id) throws UserWithIdDoesntExist {
        Optional<User> user = userRepo.findById(id);
        if (user.isPresent()) {
            return user.get();
        } else {
            throw new UserWithIdDoesntExist();
        }
    }

    @Override
    public User getUserByName(String name) throws UserWithUsernameDoesntExist {
        if (userRepo.findUserByUsername(name) != null) {
            return userRepo.findUserByUsername(name);
        } else {
            throw new UserWithUsernameDoesntExist();
        }
    }
}
